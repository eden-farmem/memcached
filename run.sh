...
then
    ...
    FSWAP_HOST_IP="192.168.100.116"    #FIXME
    HOST_NIC_PCI="0000:d8:00.1"
    RCNTRL_SSH="sc32"
    MEMSERVER_SSH=$RCNTRL_SSH
    RCNTRL_IP="192.168.100.108"
    # RCNTRL_IP="192.168.0.32"
    MEMSERVER_IP=$RCNTRL_IP
fi
...
if [[ $EDEN ]]; then
    ...
    if [[ $branch != "mcached" ]]; then
        echo "ERROR! use only the mcached branch when running memcached"
        echo "Current eden branch at ${SHENANGO_DIR}: $branch"
        exit 1
    fi
    popd
